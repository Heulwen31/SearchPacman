...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        pos = state[0]
        visistedConers = state[1]
        if pos in self.corners:
            if pos not in visistedConers:
                visistedConers.append(pos)
            return len(visistedConers) == 4
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            nodeNext = (nextx, nexty)
            hitsWall = self.walls[nextx][nexty]

            if not hitsWall:
                sucVCorners = list(visitedCorners)
                if nodeNext in self.corners:
                    if nodeNext not in sucVCorners:
                        sucVCorners.append(nodeNext)
                successor = ((nodeNext, sucVCorners), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    unvisitedCorner = [corner for corner in corners if corner not in visitedCorners]

    currentPos = node

    while unvisitedCorner:
        distance, corner = min([(util.manhattanDistance(currentPos, corner), corner) for corner in unvisitedCorner])
